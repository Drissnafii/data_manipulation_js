...
function getEmployeesByProject(arr, param) {
    const withProject = [];
    for (let i = 0; i < arr.length; i++) {
        let proj = arr[i].projects
        for (let j = 0; j < proj.length; j++) {
            if (proj[j] === param) {
                withProject.push(arr[i]);
                break;
            }
        }
    }
    if (withProject.length === 0) {
        return "No-One Have that project !, try another one.";
    }
    return [withProject, withProject.length];
}
const x = getEmployeesByProject(employees,"Projehct Gamma");
console.log(x);